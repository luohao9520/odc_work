(function (global) {
    ...
    function isDirectBookingDate(isoDate, now = new Date()) {
        return isValidIsoDate(isoDate) && isoDate >= addDaysIso(now, 1) && isoDate <= addDaysIso(now, 7);
    }
    ...
    function populateYearSelect(select, selectedYear, yearsBefore = 10, yearsAfter = 10) {
        ...
        const startYear = Math.min(currentYear - yearsBefore, targetYear);
    ...
    function normalizeHolidayName(rawName, isHoliday) {
        ...
        if (!name) return isHoliday ? '节假日' : '补班';
    ...
    function buildMonthDays(monthValue, holidays) {
        ...
        for (let day = 1; day <= daysInMonth; day += 1) {
            ...
            const isWorkdayOverride = Boolean(override && override.isHoliday === false);
    ...
    function mapTimorHolidayResponse(year, payload) {
    ...
    async function fetchChinaHolidays(year, fetchImpl) {
        ...
        if (!fetcher) throw new Error('当前环境不支持 fetch，无法自动获取节假日。');

        try {
            const timorResponse = await fetcher(`https://timor.tech/api/holiday/year/${year}`);
            if (timorResponse && timorResponse.ok) {
                const holidays = mapTimorHolidayResponse(String(year), await timorResponse.json());
                if (holidays.length) return {holidays, source: 'timor.tech 中国节假日接口'};
            }
        } catch (error) {
            // 主接口失败时继续尝试备用接口。
        }
        ...
        try {
            ...
            // 两个接口都失败时，在下方统一抛出可读错误。
        }

        throw new Error('节假日接口没有返回可用数据。');
    }
    ...
    function ensureConfirmDialog() {
        ...
        dialog.setAttribute('role', 'presentation');
        dialog.innerHTML = `
      <section class="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="confirmDialogTitle" aria-describedby="confirmDialogMessage">
        <div class="confirm-modal__halo" aria-hidden="true"></div>
        <div class="confirm-modal__head">
          <div>
    ...
    function createAdminApp() {
        ...
        const cleanupScheduledToggle = document.getElementById('cleanupScheduledToggle');
        const cleanupScheduledText = document.getElementById('cleanupScheduledText');
        const attendanceRetentionInput = document.getElementById('attendanceRetentionInput');
        const overtimeRetentionInput = document.getElementById('overtimeRetentionInput');
        ...
        function payloadFromForm() {
            return {
                scheduledEnabled: cleanupScheduledToggle.checked,
                attendanceRetentionMonths: Number(attendanceRetentionInput.value),
                overtimeRetentionMonths: Number(overtimeRetentionInput.value),
        ...
        function renderSettings(settings, scheduler) {
            cleanupScheduledToggle.checked = Boolean(settings.scheduledEnabled);
            cleanupScheduledText.textContent = settings.scheduledEnabled ? '定期清理开启' : '定期清理关闭';
            attendanceRetentionInput.value = settings.attendanceRetentionMonths;
            overtimeRetentionInput.value = settings.overtimeRetentionMonths;
            ...
            if (scheduler) {
                cleanupSchedulerStatus.textContent = scheduler.enabled ? '调度器已启动' : '调度器未启动';
                cleanupSchedulerDetail.textContent = scheduler.lastMessage || (settings.scheduledEnabled ? '等待下一次检查。' : '管理员开关关闭。');
            }
        }

        function renderCleanupResult(result) {
            cleanupResultTotal.textContent = result.message || `影响 ${result.totalDeleted || 0} 条`;
            ...
            if (result.settings) renderSettings(result.settings, result.scheduler);
        }
        ...
        function renderUsers(payload) {
            ...
            <div>
              ...
              <small class="hint">id=${user.id}${user.createdAt ? ` · ${escapeHtml(user.createdAt)}` : ''}${user.deactivatedAt ? ` · 停用${escapeHtml(user.deactivatedAt)}` : ''}</small>
            </div>
            <label class="field">
              <span>角色</span>
              <select data-role>
        ...
        `;
                ...
                toggleButton.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        const usersData = await apiFetch('/api/admin/users');
                        renderUsers(usersData);
                        userAccessStatus.textContent = `${result.user.username} 已${actionText}。`;
                    } catch (error) {
        ...
        cleanupScheduledToggle.addEventListener('change', () => {
            cleanupScheduledText.textContent = cleanupScheduledToggle.checked ? '定期清理开启' : '定期清理关闭';
        });
        ...
        previewCleanupBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                cleanupResultTotal.textContent = `预览失败：${error.message}`;
            }
        });

        runCleanupBtn.addEventListener('click', async () => {
            const confirmed = await confirmAction('将按当前保留策略永久删除过期数据，并尝试回收 SQLite 空间。建议先点击“预览清理”。确定继续吗？', {
                title: '立即清理过期数据',
                confirmText: '确认清理',
                tone: 'danger',
            });
    ...
    function createAttendanceApp() {
        ...
        function applyRulesVisibility(visible) {
            ...
            showRulesText.textContent = visible ? '显示' : '隐藏';
            rulePanel.classList.toggle('hidden', !visible);
        ...
        function includePastDates() {
            return Boolean(includePastToggle && includePastToggle.checked);
        }
        ...
        function selectedStrategyLabel() {
            if (!smartStrategySelect) return '每周均衡达标';
        ...
        function render() {
            ...
            days.forEach((day, index) => {
                ...
                node.querySelectorAll('button').forEach((button) => {
                    ...
                    button.addEventListener('click', async () => {
                        const nextStatus = state.selections[day.iso] === buttonStatus ? null : buttonStatus;
            ...
            remainingDays.textContent = result.remainingDays === 0 ? '已满足最低要求' : `还需 ${result.remainingDays} 天`;
            ...
            formulaText.textContent = [
                ...
                '分母包含：公司打卡、居家办公、未选的可统计工作日；请假按休息日处理，不计入分母；分子只包含公司打卡。',
            ].join('\n');
        ...
        smartScheduleBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                await loadMonth();
                const recommendationCount = Array.isArray(data.recommendedDates) ? data.recommendedDates.length : 0;
                const recommendationText = recommendationCount ? `，算法推荐 ${recommendationCount} 个公司日` : '';
                holidaySummary.textContent = `智能排班完成：使用⌈${data.strategyLabel}⌋ ${recommendationText}，${data.weeklyPlan.length} 个周段已尽量满足每周 ${state.targetRate}% 公司打卡。`;
            } finally {
    ...
    function createHolidayApp() {
        ...
        async function loadSourceUsers() {
            ...
            data.users.forEach((user) => {
                ...
                option.textContent = `${user.username}（${user.overrideCount} 条配置）`;
    ...
    function createApiDocsApp() {
        ...
        const endpoints = [
            ...
            {method: 'POST', path: '/api/attendance/smart-schedule', title: '智能排班', description: '默认只排明天及之后；按所选星期组合尽量满足每周目标公司打卡比例。', body: {month: '2026-07', strategy: 'recommended', includePast: false}},
            {method: 'GET', path: '/api/holidays?year=2026', title: '读取年度节假日', description: '返回休息日、补班日、覆盖规则和完整年度日历。'},
            ...
            {method: 'DELETE', path: '/api/admin/users/1', title: '删除用户', description: '管理员永久删除用户及其业务数据；执行前建议备份 SQLite。', body: {}},
        ...
        async function sendRequest(method, path, bodyText, output) {
            ...
            if (method !== 'GET' && bodyText.trim()) {
                try {
                    ...
                    output.textContent = `请求体不是合法 JSON：${error.message}`;
                    return;
                }
            } else if (method !== 'GET') {
                options.body = '{}';
    ...
    function createSeatBookingApp() {
        ...
        function populateSeatCalendarMonths(selectedMonth) {
        ...
        function renderCalendar() {
            ...
            const {year, month} = splitMonthValue(calendarMonth);
            ...
            for (let index = 0; index < 42; index += 1) {
                ...
                if (bookedSeat) {
                    ...
                    if (bookedSeat.status !== 'success') {
                        ...
                        bookNow.title = bookNow.disabled ? '直接预约只能选择明天至 7 天后的日期' : '提交到外部平台';
        ...
        function renderSchedulerStatus(scheduler) {
            ...
            const enabledText = scheduler.enabled || scheduler.envEnabled ? '内置调度器已启用' : '内置调度器未启用';
        ...
        function renderSeats() {
            ...
            if (seatListLoading) {
                ...
                seatList.innerHTML = '<p class="empty-note">正在读取座位列表，请稍候…</p>';
                return;
            }
            if (seatListError) {
            ...
            seats.forEach((seat) => {
                ...
                <div class="seat-card-actions">
                    ...
                    <button type="button" class="primary-button" data-action="direct" ${seat.available && isDirectBookingDate(selectedBookingDate) ? '' : 'disabled'}>直接预约</button>
                </div>
            `;
                card.querySelector('[data-action="plan"]').addEventListener('click', async () => {
                    if (!selectedBookingDate) return;
                    const data = await apiFetch('/api/seat-booking/plans', {
                        method: 'PUT',
        ...
        async function init() {
            ...
            if (bookingTimeInput) bookingTimeInput.addEventListener('change', renderCalendar);
            if (syncSeatBookingsBtn) syncSeatBookingsBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    renderCalendar();
                    statusText.textContent = `同步完成：新增 ${data.synced || 0} 条，更新日历 ${data.updated || 0} 条。`;
                } catch (error) {

...
def due_date_time_for(booking_date: str, booking_time: str, advance_days: int) -> datetime:
...
def validate_booking_schedule(booking_date: str, booking_time: str, advance_days: int, enabled: bool, now: datetime | None = None) -> None:
    ...
    if enabled and due_date_time_for(booking_date, booking_time, advance_days) < now.replace(second=0, microsecond=0):
        raise ValueError("提前预约的座位可能不会早于当前时间，请调整预约日期、提前天数或定时提交时间。")
...
def seat_sort_key(seat_id: str) -> tuple:
    match = re.match(r"^(\d+)(\d+)$", seat_id or "")
    if not match:
...
def choose_booking_seat(client: SeatBookingClient, booking_date: str, preferred_seat_id: str, preferred_seat_name: str = "") -> tuple[str, str, str]:
    ...
    if not seats:
        return preferred_seat_id, preferred_seat_name, "未获取到可用座位，按首选座位提交。"
    for seat in seats:
        if seat["id"] == preferred_seat_id:
            return seat["id"], seat.get("name") or seat["id"], "首选座位可用。"
    preferred_key = seat_sort_key(preferred_seat_id)
    fallback = min(seats, key=lambda s: abs(seat_sort_key(s["id"])[1] - preferred_key[1]) if seat_sort_key(s["id"])[0] == preferred_key[0] else 10000 + seat_sort_key(s["id"])[1])
...
def read_secret_settings(user_id: int) -> dict:
    ...
    if not row:
        return {}
    settings = dict(row)
    settings["external_password"] = decrypt_secret(settings["external_password"])
    return settings
...
def read_plans_for_month(user_id: int, month: str) -> dict:
    rows = get_db().execute(
        "SELECT booking_date, seat_id, seat_name, status, message, updated_at FROM seat_booking_plans WHERE user_id = ? AND booking_date LIKE ? ORDER BY id DESC",
        (user_id, f"{month}-%")
    ).fetchall()
    plans = {}
    for row in rows:
        plans[row["booking_date"]] = {
            "bookingDate": row["booking_date"],
            "seatId": row["seat_id"],
            "seatName": row["seat_name"],
            "status": row["status"],
...
def sync_platform_bookings(user_id: int, current_date=None) -> dict:
    ...
    for booking in bookings:
        ...
        db.execute(
            ...
            (user_id, booking_date, seat_id, seat_name, "success", "已从外部平台同步更新预约记录。", now_iso())
        )
        updated += 1
    db.commit()
...
def normalize_settings_payload(payload: dict, current: dict | None = None) -> dict:
    ...
    if booking_date and not valid_date(booking_date):
        raise ValueError("预约日期格式为 YYYY-MM-DD")
    if not valid_time(booking_time):
        raise ValueError("时间格式为 HH:MM")
    advance_days = clamp_advance_days(payload.get("advanceDays", 0))
    enabled = 1 if payload.get("enabled", current.get("enabled", 0)) else 0
    if booking_date:
...
def save_plan(user_id: int, booking_date: str, seat_id: str, seat_name: str, settings: dict) -> dict:
    ...
    db.execute(
        "INSERT INTO seat_booking_plans(user_id, booking_date, seat_id, seat_name, status, message, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) "
        "ON CONFLICT(user_id, booking_date) DO UPDATE SET seat_id = excluded.seat_id, seat_name = excluded.seat_name, status = excluded.status, message = excluded.message, updated_at = excluded.updated_at",
        (user_id, booking_date, seat_id, seat_name, "pending", message, now_iso())
    )
    db.commit()
...
def book_with_settings(user_id: int, booking_date: str | None = None, seat_id: str | None = None, seat_name: str | None = None, direct: bool = False) -> dict:
    ...
    validate_booking_schedule(target_date, "00:00", 0, False)  # 仅检查日期有效性
    if direct:
    ...
    if successful_booking_count_for_week(user_id, target_date) >= 3:
        message = "本周已有 3 次座位预约成功，本次不再自动预约。"
        run = save_run(user_id, target_date, target_seat_id, target_seat_name, "skipped", message)
        return {"ok": True, "run": run}

    try:
        client = platform_login(settings)
        target_seat_id, target_seat_name, choice_msg = choose_booking_seat(client, target_date, target_seat_id, target_seat_name)
        result = client.book_seat(target_date, target_seat_id, target_seat_name)
        message = str(result.get("message")) or "预约请求已提交。"
        if target_seat_name and target_seat_name not in message:
            message = f"{target_seat_name} 已预约成功"
...
@seat_booking_bp.get("/seat-bookings")
@login_required
...
@seat_booking_bp.get("/seat-bookings/scheduler")
@login_required
...
def update_seat_booking_settings():
    ...
    db.execute(
        ...
        (
            ...
            encrypt_secret(normalized["external_password"]) if normalized["external_password"] else "",
...
def fetch_seats():
    ...
    if not valid_date(booking_date):
        return jsonify({"error": "预约日期格式为 YYYY-MM-DD"}), 400
    try:
        client = platform_login(settings)
        result = client.get_seat_list(booking_date)
...
@seat_booking_bp.delete("/seat-bookings/plans")
@login_required
    ...
    if successful:
        ...
        try:
            ...
            if result.get("ok") is False:
                return jsonify({"error": "外部平台取消预约失败。"}), 502
            external_cancelled = True
        except Exception:
            return jsonify({"error": "外部平台取消预约失败。"}), 502
    db = get_db()
    db.execute("DELETE FROM seat_booking_runs WHERE user_id = ? AND booking_date = ?", (user_id, booking_date))
    db.execute("DELETE FROM seat_booking_plans WHERE user_id = ? AND booking_date = ? AND status = 'success'", (user_id, booking_date))
    db.execute(
        "INSERT INTO seat_booking_runs(user_id, booking_date, seat_id, seat_name, status, message, run_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def book_seat():
    ...
    try:
        result = book_with_settings(
            user_id,
            str(payload.get("bookingDate") or None) or None,
            str(payload.get("seatId") or None) or None,
            str(payload.get("seatName") or None) or None,
            direct=True,
        )
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
...
def run_due_seat_bookings(current_time: datetime | None = None) -> list[dict]:
    ...
    for row in rows:
        user_id = row["user_id"]
        try:
            due_at = due_date_time_for(row["booking_date"], row["booking_time"], row["advance_days"])
        except ValueError:
            continue
        if current_time.replace(second=0, microsecond=0) < due_at.replace(second=0, microsecond=0):
            continue
        existing = db.execute(
            "SELECT id FROM seat_booking_runs WHERE user_id = ? AND booking_date = ? AND status IN ('success', 'limited') LIMIT 1",
            (user_id, row["booking_date"])
        ).fetchone()
        if existing:
            continue

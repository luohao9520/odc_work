...
- 出勤统计由 Flask 后端统一计算并返回 `api/attendance` 返回，前端只负责展示。
...
使用 Waitress 时不要用 `waitress-serve run:app`，因为 `run.py` 中的模块级 `app` 会禁用内置调试器。请使用 `--call backend.app.main:create_app`。
...
页面可以正常访问，但多数手机浏览器不会启用 Service Worker，因此离线缓存和标准安装能力可能不可用。短期可以先作为普通手机网页使用，如果后续要稳定 PWA 安装和高线能力，建议配置 HTTPS。
...
用户手动选择的日期会被保留，不会被智能排班或“工作日全设为公司”覆盖。
“工作日全设为公司”和“智能排班”默认只调整明天及之后的非手动选择日期；勾选“包含过去日期”后才会覆盖整个月的非手动选择日期。

智能排班以一周为周期，优先让每个周期也满足目标公司打卡比例；由于每周独立向上取整，最终公司打卡天数可能略高于月度最低要求。

可选择排班方式包括：智能排班、周一三优先、周二四优先、周三周五优先、周三四五优先、周一三五分散。

智能排班使用日期预演优化，根据历史和当前的手动公司/居家记录建立近期加权偏好画像，对每个候选工作日预测公司打卡适合度，并在每周达标、手动保护、请假排除和分散排班约束下生成推荐日期。

样本不足时使用温和无先验生成、分散的安排。

## 座位预约

“座位预约”页面会连接外部平台：

- 登录：`http://meet.fxodc.club.com.cn/Mobile/Login/Login`
- 座位列表：`http://meet.fxodc.club.com.cn/Mobile/Seat/getSeatList`
- 提交预约：`http://meet.fxodc.club.com.cn/Mobile/Seat/bookSeat`

平台可用密码保存在本地 SQLite，仅用于调用外部预约接口；读取接口 API 只返回 `hasPassword`，不会返回密码。

预约目标日期保存在月历页，月历支持切换月份。同月中其他未预约日期以灰色状态显示，点击后会跳转到对应月份并选中该日期。

点击月日历日期会直接打开日历区间内该日期的预约选项，座位列表支持按座位号关键字筛选，只显示可用座位。点击某个座位即确认该日期的预约计划。

日预约状态会显示“未预约 / 预约中 / 成功”，表示已保存计划等待定时提交。

预约成功日期会显示“取消预约”按钮，点击后需要确认；取消后会移除本地成功记录，该日期不再计入同周成功预约上限。

“定时提交时间”和“提前预约天数”统一配置在月历页，提前天数范围为 0 到 7 天。保存配置后，调度器会按 `预约日期 - 提前天数 > 定时提交时间` 判断是否需要自动提交。

不支持预约当天日期之前的日期，启用定时预约时，`预约日期 - 提前天数 + 定时提交时间` 也不能早于当前日期。

自动预约前会依次尝试获取目标座位列表；如果首选座位已不可用，会按座位号接近最优可预约的座位。

同一自然周最多保留 3 天成功预约。如果该周已有 3 天成功预约，调度器不会继续为同周其他计划提交预约，除非先取消某天成功预约。

预约成功后，座位号会显示在座位预约日历日期和出勤登记日历对应日期中。

外部平台反馈“预定次数不可超过 3 次”，系统会记录为 `Limited`，视为正常错误结果。

Flask 服务默认启用内置轻量调度器，调度器使用 SQLite 预约排程多进程重启执行风险。默认每 60 秒检查一次：
...
```json
{
  "https": "http://你的用户名:你的密码@ntproxy1.hk.hsbc:8080",
  "http": "http://你的用户名:你的密码@ntproxy1.hk.hsbc:8080",
  "no_proxy": "*.hsbc"
}
```
...
如果部署到生产环境后，如果节假日 API 和座位预约平台都可以直接访问，可以不创建 `instance\proxy.json`，也不要设置 `ATTENDANCE_HTTPS_PROXY` / `ATTENDANCE_HTTP_PROXY`。

如果运行环境里存在系统代理变量但本应用需要强制直连，可以设置：
...
清理会删除过期业务记录，并可执行 SQLite `VACUUM` 回收空间。206 小硬盘部署时，建议开启该开关并定期备份。

...
- 真实代理配置放在 `instance/proxy.json`，该文件被 `./instance/proxy.example.json`。
- 日期计算和节假日获取放在 `backend/app/backend/calendar_utils.py`，便于单元测试。
- 出勤统计和日志放在 `backend/app/domain/attendance/calculator.py`，由后端 API 返回或 `summary`，前端只展示结果。
